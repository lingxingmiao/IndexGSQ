...
def _GSQ_K编码_Numba(数组, 向量块, 最大量级):
    ...
    for g in range(组数):
        for d in range(维度):
            ...
            for r in range(向量块):
                row_idx = g * 向量块 + r
                if row_idx < 行数: val = 数组[row_idx, d]
                else: val = 0.0
...
def 向量重排(数组, 聚类块大小, 重排乘数):
    ...
    while 已处理数量 < 向量数量:
        ...
        有效索引 = 候选索引[有效掩码]
        if len(有效索引) == 0: 有效索引 = np.array([搜索指针])
        候选向量组 = 数组_归一[有效索引]
    ...
    if 向量数量 <= 65535: 数据类型 = np.uint16
    elif 向量数量 <= 4294967296: 数据类型 = np.uint32
    else: 数据类型 = np.uint64
...
def 编码(self, 数组, 位深):
    ...
    for 起始 in range(0, 总行数, self.Config.INDEX_GSQ_RERANKER_BLOCK_SIZE):
        ...
        if 位深 == 8: 压缩 = 量化值
        elif 位深 == 6: 压缩 = 加速打包6(量化值)
        elif 位深 == 4: 压缩 = 加速打包4(量化值)
        elif 位深 == 3: 压缩 = 加速打包3(量化值)
        elif 位深 == 2: 压缩 = 加速打包2(量化值)
...
def 重排列表(原始列表, 映射表):
    if 原始列表 is None: return None
    return [原始列表[i] for i in 映射表]

def _解包(数据包, 位深, 行数, 维度):
    if 位深 == 8: return 数据包["packed"]
    elif 位深 == 6: return 加速解包6(数据包["packed"], 行数 * 维度)
    elif 位深 == 4: return 加速解包4(数据包["packed"], 行数 * 维度)
    elif 位深 == 3: return 加速解包3(数据包["packed"], 行数 * 维度)
    elif 位深 == 2: return 加速解包2(数据包["packed"], 行数 * 维度)

class IndexGSQKCosine:
    def __init__(self, vectors_block=128, reranker_block=128, reranker_factor=4, quantization: int = 2):
        self.向量库 = []
        self.映射表 = []
        self.位深 = quantization
        self.模式 = "IndexGSQKCosine"
        self.Config = SimpleNamespace(
            INDEX_GSQ_BLOCK_SIZE = vectors_block,
            INDEX_GSQ_RERANKER_BLOCK_SIZE = reranker_block,
            INDEX_GSQ_RERANKER_FACTOR = reranker_factor
        )
...
class IndexGSQKCosineFast:
    ...
    def search(self, query, k):
        ...
        for 数据包 in self.向量库:
            行数, 维度 = 数据包["shape"]
            缩放值 = (np.asarray(数据包["scales"]).view(np.int16).astype(np.float32) / 32768.0 * 数据包["max_scale"]).reshape(-1, 维度)
            最小值 = (np.asarray(数据包["mins"]).view(np.int16).astype(np.float32) / 32768.0 * 数据包["max_min"]).reshape(-1, 维度)
            位深 = 数据包["bit_depth"]
...
class IndexGSQKCosineMoE:
    def __init__(self, vectors_block=128, reranker_block=128, reranker_factor=4, quantization: int = 2, 
                 moe_block_size=32, moe_spl_svd=5, moe_km_lm=16, moe_spl_lm=5, moe_exp=0.1):
        self.向量库 = []
        self.映射表 = []
        self.位深 = quantization
        ...
        self.Config = SimpleNamespace(
            ...
            INDEX_GSQ_RERANKER_BLOCK_SIZE = reranker_block,
            INDEX_GSQ_RERANKER_FACTOR = reranker_factor,
            INDEX_GSQ_MOE_BLOCK_SIZE = moe_block_size,
            INDEX_GSQ_MOE_SPL_SVD = moe_spl_svd,
            INDEX_GSQ_MOE_KM_LM = moe_km_lm,
            INDEX_GSQ_MOE_SPL_LM = moe_spl_lm,
            INDEX_GSQ_MOE_EXP = moe_exp
        )
    ...
    def save(self, filename: str):
        with open(filename, 'wb') as f:
            pickle.dump({
                ...
                "exp": self.exp,
                "已训练": self.已训练,
                "旋转块矩阵": self.旋转块矩阵.astype(np.float16) if self.旋转块矩阵 is not None else None,
                "LM中心": self.LM中心,
                "LM边界": self.LM边界,
                "Config": vars(self.Config)
            }, f, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def add(self, vectors):
        ...
        moe_exp_cfg = self.Config.INDEX_GSQ_MOE_EXP
        self.exp = max(1, int(moe_exp_cfg) if isinstance(moe_exp_cfg, (int, np.uint32)) or moe_exp_cfg > 1 else int(len(self.向量库) * moe_exp_cfg))
...
def load(filename: str):
    ...
    mode = d.pop("模式", "IndexGSQKCosine")
    index = globals()[mode]()
    if config_data:
        index.Config = SimpleNamespace(**config_data)
    for k, v in d.items():
        setattr(index, k, v)
    return index
